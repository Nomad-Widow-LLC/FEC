...
export default function App() {
      ...
      .then(async (productList) => {
        let images = await Promise.all(
          productList.map((product) =>
            axios.get(`/styles?product_id=${product.id}`)
          )
        );
        let placeholder =
          "https://s3.amazonaws.com/media.thecrimson.com/photos/2022/04/20/122908_1355986.jpg";
        let pictures = [];
        ...
        for (let i = 0; i < images.length; i++) {
          ...
          if (images[i].data.results.length === 0) {
            ...
            pic.thumbnail_url =
              images[i].data.results[0].photos[0].thumbnail_url;
          }
          pictures.push(pic);
        }
        return pictures;
  ...
  return (
    <>
      ...
      >
        {homepage ? (
          <Homepage productPics={productPics} getProduct={getProduct} />
        ) : (
          <div>
